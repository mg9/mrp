...
function iterate(d::Dataset, state=collect(1:d.totalbatch))
    ...
    return  ((src, src_mask, tgt, src_attention_maps, tgt_attention_maps, generate_targets, src_copy_targets, tgt_copy_targets, edge_heads, edgelabels, corefs, parser_mask) , new_state)
end
...
function trainmodel(epochs)
    ...
    epoch = adam(model, (( src, src_mask, tgt, src_attention_maps, tgt_attention_maps, generate_targets, src_copy_targets, tgt_copy_targets, edge_heads, edgelabels, corefs, parser_mask) for (src, src_mask, tgt, src_attention_maps, tgt_attention_maps, generate_targets, src_copy_targets, tgt_copy_targets, edge_heads, edgelabels, corefs, parser_masks) in trnbatches))
    ctrn = collect(trn)
